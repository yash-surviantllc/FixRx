...
const processNotificationJob = async (job: any) => {
  ...
  logger.info(`Processing notification job ${job.id} for user ${userId || 'topic: ' + topic}`);
  ...
  try {
    ...
    if (token) {
      ...
      result = await FirebaseService.sendToToken(token, payload);
    } else if (tokens && tokens.length > 0) {
      // Send to multiple tokens
      result = await FirebaseService.sendToTokens(tokens, payload);
    } else if (topic) {
      // Send to topic
      result = await FirebaseService.sendToTopic(topic, payload);
    } else {
