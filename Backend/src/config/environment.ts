...
const envSchema = Joi.object({
  ...
  // Email Templates
  EMAIL_TEMPLATE_INVITATION: Joi.string().allow('').optional(),
  EMAIL_TEMPLATE_WELCOME: Joi.string().allow('').optional(),
  EMAIL_TEMPLATE_PASSWORD_RESET: Joi.string().allow('').optional(),
...
export const config = {
  ...
  // CORS origins based on environment with explicit validation
  corsOrigins: (() => {
    if (env.NODE_ENV === 'production') {
      // In production, use environment variable or fallback to secure defaults
      const prodOrigins = process.env.CORS_ALLOWED_ORIGINS?.split(',').map(origin => origin.trim()) || 
                         ['https://fixrx.com', 'https://app.fixrx.com'];
      return prodOrigins;
    } else if (env.NODE_ENV === 'test') {
      // Minimal origins for testing
