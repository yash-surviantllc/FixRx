...
export class VendorController {
  ...
  static async searchVendors(req: Request, res: Response): Promise<void> {
    ...
    if (latitude && longitude) {
      ...
      if (isNaN(lat) || isNaN(lng) || isNaN(radiusKm)) {
        throw new AppError('Invalid numeric values for coordinates or radius', 400);
      }
      ...
      if (radiusKm < 1 || radiusKm > 500) {
        throw new AppError('Search radius must be between 1 and 500 kilometers', 400);
      }
    ...
    if (serviceCategories) {
      const categories = Array.isArray(serviceCategories) 
        ? serviceCategories.map(cat => String(cat))
  ...
  static async createVendorProfile(req: Request, res: Response): Promise<void> {
    ...
    logger.info(`Vendor profile created and user activated: ${userId}`);
  ...
  static async getNearbyVendors(req: Request, res: Response): Promise<void> {
    ...
    }).filter((vendor: any) => !vendor.distance || vendor.distance <= radiusKm);
