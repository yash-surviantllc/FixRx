  ...
  setupSearchRoutes() {
    ...
    this.app.post('/api/v1/search/vendors', optionalAuth, async (req, res, next) => {
      try {
        ...
        if (req.user) {
          await monitoringService.trackUserActivity(req.user.id, 'vendor_search', {
            userType: req.user.role,
            searchRadius: searchParams.radiusKm
          });
        }

        const searchResults = await geoSearchService.searchVendors(searchParams);
        
        // Track business metric
        await monitoringService.trackBusinessMetric('vendor_searches', 1, {
          resultsCount: searchResults.vendors.length,
    ...
    this.app.get('/api/v1/search/nearby', optionalAuth, async (req, res, next) => {
      try {
        ...
        const vendors = await geoSearchService.findNearbyVendors(
          parseFloat(lat), 
          parseFloat(lng), 
          parseInt(radius), 
          parseInt(limit)
